...
class ScanEventsViewController: PopoverVC, ProgressBarDelegate {
    ...
    func refreshEventList() {
        ...
        if !searchBarText.isEmpty {
            events = self.events.filter() {
                event in
                if event.eventName.lowercased().range(of: searchBarText.lowercased()) != nil { return true } else { return false }
            }
        }
    ...
    func findEvents() {
        ...
        for i in from...to { arrayOfEventsToBeSearched.append(i) }
        ...
        if arrayOfEventsToBeSearched.count != 0 {
            ...
            eventsTimer = Foundation.Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(checkIfEventDidGetName(_:)), userInfo: firstEventIndexThatDontHaveName, repeats: false)
    ...
    @objc func checkIfEventDidGetName (_ timer:Foundation.Timer) {
        ...
        guard let eventIndex = timer.userInfo as? Int else { return }
        
        timesRepeatedCounter += 1
        if timesRepeatedCounter < 3 {
            eventsTimer = Foundation.Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(checkIfEventDidGetName(_:)), userInfo: eventIndex, repeats: false)
            ...
            if let indexOfEventIndexInArrayOfNamesToBeSearched = arrayOfEventsToBeSearched.index(of: eventIndex){ // Get the index of received timerId. Array "arrayOfNamesToBeSearched" contains indexes of devices that don't have name
                if indexOfEventsToBeSearched + 1 < arrayOfEventsToBeSearched.count{ // if next exists
                    ...
                    timesRepeatedCounter = 0
                    eventsTimer = Foundation.Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(checkIfEventDidGetName(_:)), userInfo: nextEventIndexToBeSearched, repeats: false)
    ...
    @objc func nameReceivedFromPLC (_ notification:Notification) {
        if Foundation.UserDefaults.standard.bool(forKey: UserDefaults.IsScaningEventsNameAndParameters) {
            ...
            guard let indexOfEventIndexInArrayOfNamesToBeSearched = arrayOfEventsToBeSearched.index(of: timerIndex) else { return }
...
extension ScanEventsViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let cell = tableView.dequeueReusableCell(withIdentifier: "eventsCell") as? EventsCell {
            ...
            cell.setCell(event: events[indexPath.row])
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        didSelect(event: events[indexPath.row])
    }
    ...
    func didSelect(event: Event) {
        ...
        defaultImageTwo = event.eventImageTwoDefault
        customImageTwo = event.eventImageTwoCustom
        imageDataTwo = nil
        ...
        if let id = event.eventImageOneCustom {
            if let image = DatabaseImageController.shared.getImageById(id) {
                ...
                } else { if let defaultImage = event.eventImageOneDefault { imageSceneOne.image = UIImage(named: defaultImage) } }
        ...
        if let id = event.eventImageTwoCustom {
            if let image = DatabaseImageController.shared.getImageById(id) {
                ...
                } else { if let defaultImage = event.eventImageTwoDefault { imageSceneTwo.image = UIImage(named: defaultImage) } }
