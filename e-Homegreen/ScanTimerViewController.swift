...
enum TimerType:Int{
    ...
    var description: String {
        ...
        case .stopwatch: return "Stopwatch/User"
        }
    }
...
class ScanTimerViewController: PopoverVC, ProgressBarDelegate {
    ...
    func setupViews() {
        ...
        imageTimerOne.tag = 1
        imageTimerOne.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(handleTap(_:))))
    ...
    func refreshTimerList() {
        ...
        if !searchBarText.isEmpty{
            timers = self.timers.filter() {
                timer in
                if timer.timerName.lowercased().range(of: searchBarText.lowercased()) != nil { return true } else { return false }
            }
        }
    ...
    func addObservers(){
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(nameReceivedFromPLC(_:)), name: NSNotification.Name(rawValue: NotificationKey.DidReceiveTimerFromGateway), object: nil)
    ...
    @IBAction func btnTimerType(_ sender: UIButton) {
        button = sender
        var popoverList:[PopOverItem] = []
        for item in TimerType.allItem { popoverList.append(PopOverItem(name: item.description, id: "\(item.rawValue)")) }
        openPopover(sender, popOverList:popoverList)
    }
    ...
    func findNames() {
        ...
        if arrayOfNamesToBeSearched.count != 0 {
            ...
            timerNameTimer = Foundation.Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(checkIfTimerDidGetName(_:)), userInfo: firstTimerIndexThatDontHaveName, repeats: false)
    ...
    func checkIfTimerDidGetName (_ timer:Foundation.Timer) {
        ...
        if timesRepeatedCounterNames < 3 {
            timerNameTimer = Foundation.Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(checkIfTimerDidGetName(_:)), userInfo: timerIndex, repeats: false)
            ...
            if let indexOfTimerIndexInArrayOfNamesToBeSearched = arrayOfNamesToBeSearched.index(of: timerIndex) { // Get the index of received timerId. Array "arrayOfNamesToBeSearched" contains indexes of devices that don't have name
                if indexOfTimerIndexInArrayOfNamesToBeSearched+1 < arrayOfNamesToBeSearched.count { // if next exists
                    ...
                    timesRepeatedCounterNames = 0
                    timerNameTimer = Foundation.Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(checkIfTimerDidGetName(_:)), userInfo: nextTimerIndexToBeSearched, repeats: false)
    ...
    func findParametarsForTimer() {
        ...
        for i in from...to {
            for timerTemp in timers { if timerTemp.timerId.intValue == i { arrayOfParametersToBeSearched.append(i) } }
        }
        ...
        if arrayOfParametersToBeSearched.count != 0 {
            ...
            timerParameterTimer = Foundation.Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(checkIfTimerDidGetParametar(_:)), userInfo: parameterIndex, repeats: false)
    ...
    func checkIfTimerDidGetParametar (_ timer:Foundation.Timer) {
        ...
        if timesRepeatedCounterParameters < 3 {
            timerParameterTimer = Foundation.Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(checkIfTimerDidGetParametar(_:)), userInfo: timerIndex, repeats: false)
            ...
            if let indexOfTimerIndexInArrayOfParametersToBeSearched = arrayOfParametersToBeSearched.index(of: timerIndex) { // Get the index of received timerId. Array "arrayOfNamesToBeSearched" contains indexes of devices that don't have name
                if indexOfTimerIndexInArrayOfParametersToBeSearched+1 < arrayOfParametersToBeSearched.count { // if next exists
                    ...
                    timesRepeatedCounterParameters = 0
                    timerParameterTimer = Foundation.Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(checkIfTimerDidGetParametar(_:)), userInfo: nextTimerIndexToBeSearched, repeats: false)
...
extension ScanTimerViewController: UITableViewDataSource, UITableViewDelegate {
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        didSelect(timer: timers[indexPath.row])
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let cell = tableView.dequeueReusableCell(withIdentifier: "timersCell") as? TimersCell {
            ...
            cell.setCell(timer: timers[indexPath.row])
    ...
    func didSelect(timer: Timer) {
        ...
        defaultImageTwo = timer.timerImageTwoDefault
        customImageTwo = timer.timerImageTwoCustom
        imageDataTwo = nil
        
        if let id = timer.timerImageOneCustom {
            if let image = DatabaseImageController.shared.getImageById(id) {
                
                if let data =  image.imageData { imageTimerOne.image = UIImage(data: data)
                } else { if let defaultImage = timer.timerImageOneDefault { imageTimerOne.image = UIImage(named: defaultImage) } }
        ...
        if let id = timer.timerImageTwoCustom {
            if let image = DatabaseImageController.shared.getImageById(id) {
                ...
                if let data =  image.imageData { imageTimerTwo.image = UIImage(data: data)
                } else { if let defaultImage = timer.timerImageTwoDefault { imageTimerTwo.image = UIImage(named: defaultImage) } }
