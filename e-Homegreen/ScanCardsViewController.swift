...
class ScanCardsViewController: UIViewController, ProgressBarDelegate {
    ...
    func findNames() {
        ...
        if let rangeFromText = fromTextField.text, let rangeFrom = Int(rangeFromText) { from = rangeFrom }
        if let rangeToText = toTextField.text, let rangeTo = Int(rangeToText) { to = rangeTo }
        
        if to < from { self.view.makeToast(message: "Range is not properly set"); return }
        for i in from...to { arrayOfNamesToBeSearched.append(i) }
    ...
    @objc func checkIfCardDidGetName (_ timer:Foundation.Timer) {
        ...
        guard let timerIndex = timer.userInfo as? Int else { return }
        
        timesRepeatedCounterNames += 1
        if timesRepeatedCounterNames < 3 {
            cardNameTimer = Foundation.Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(checkIfCardDidGetName(_:)), userInfo: timerIndex, repeats: false)
            ...
            if let indexOfTimerIndexInArrayOfNamesToBeSearched = arrayOfNamesToBeSearched.index(of: timerIndex) { // Get the index of received cardId. Array "arrayOfNamesToBeSearched" contains indexes of devices that don't have name
                if indexOfTimerIndexInArrayOfNamesToBeSearched + 1 < arrayOfNamesToBeSearched.count { // if next exists
                    ...
                    timesRepeatedCounterNames = 0
                    cardNameTimer = Foundation.Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(checkIfCardDidGetName(_:)), userInfo: nextTimerIndexToBeSearched, repeats: false)
    ...
    @objc func cardNameReceivedFromPLC (_ notification:Notification) {
        if Foundation.UserDefaults.standard.bool(forKey: UserDefaults.IsScaningCardNames) {
            ...
            guard let indexOfDeviceIndexInArrayOfNamesToBeSearched = arrayOfNamesToBeSearched.index(of: timerIndex) else { return } // Array "arrayOfNamesToBeSearched" contains indexes of devices that don't have name
    ...
    @objc func findParametarsForCard() {
        ...
        for i in from...to {
            for cardTemp in cards { if cardTemp.id.intValue == i { arrayOfParametersToBeSearched.append(i) } }
        }
        ...
        if arrayOfParametersToBeSearched.count != 0 {
            ...
            cardParameterTimer = Foundation.Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(checkIfCardDidGetParametar(_:)), userInfo: parameterIndex, repeats: false)
    ...
    @objc func checkIfCardDidGetParametar (_ timer:Foundation.Timer) {
        ...
        // If not tree times, send same command again
        // If three times reached, search for next timer ID if it exists
        guard let cardIndex = timer.userInfo as? Int else { return }
        
        timesRepeatedCounterParameters += 1
        if timesRepeatedCounterParameters < 3 {
            cardParameterTimer = Foundation.Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(checkIfCardDidGetParametar(_:)), userInfo: cardIndex, repeats: false)
            ...
            if let indexOfTimerIndexInArrayOfParametersToBeSearched = arrayOfParametersToBeSearched.index(of: cardIndex){ // Get the index of received timerId. Array "arrayOfNamesToBeSearched" contains indexes of devices that don't have name
                if indexOfTimerIndexInArrayOfParametersToBeSearched + 1 < arrayOfParametersToBeSearched.count{ // if next exists
                    ...
                    timesRepeatedCounterParameters = 0
                    cardParameterTimer = Foundation.Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(checkIfCardDidGetParametar(_:)), userInfo: nextTimerIndexToBeSearched, repeats: false)
    ...
    @objc func cardParametarReceivedFromPLC (_ notification:Notification) {
        if Foundation.UserDefaults.standard.bool(forKey: UserDefaults.IsScaningCardParameters) {
            ...
            guard let indexOfDeviceIndexInArrayOfParametersToBeSearched = arrayOfParametersToBeSearched.index(of: cardIndex) else { return } // Array "arrayOfNamesToBeSearched" contains indexes of devices that don't have name
    ...
    @objc func timerParametarReceivedFromPLC (_ notification:Notification) {
        if Foundation.UserDefaults.standard.bool(forKey: UserDefaults.IsScaningTimerParameters) {
            guard let info = notification.userInfo! as? [String:Int] else { return }
            guard let timerIndex = info["timerId"] else { return }
            guard let indexOfDeviceIndexInArrayOfParametersToBeSearched = arrayOfParametersToBeSearched.index(of: timerIndex) else { return } // Array "arrayOfNamesToBeSearched" contains indexes of devices that don't have name
    ...
    func dismissScaningControls() {
        ...
        if !shouldFindCardParameters && !shouldFindTimerNames && !shouldFindTimerParameters {
            ...
            if shouldFindCardParameters { _ = Foundation.Timer.scheduledTimer(timeInterval: 1.5, target: self, selector: #selector(findParametarsForCard), userInfo: nil, repeats: false) }
            if shouldFindTimerNames { _ = Foundation.Timer.scheduledTimer(timeInterval: 1.5, target: self, selector: #selector(findTimerNames), userInfo: nil, repeats: false) }
            if shouldFindTimerParameters{ _ = Foundation.Timer.scheduledTimer(timeInterval: 1.5, target: self, selector: #selector(findParametarsForTimer), userInfo: nil, repeats: false) }
        }
    }
    
    func addObservers() {
        // Notification that tells us that card is received and stored
        NotificationCenter.default.addObserver(self, selector: #selector(cardNameReceivedFromPLC(_:)), name: NSNotification.Name(rawValue: NotificationKey.DidReceiveCardFromGateway), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(cardParametarReceivedFromPLC(_:)), name: NSNotification.Name(rawValue: NotificationKey.DidReceiveCardParameterFromGateway), object: nil)
        // Notification that tells us that timer is received and stored
        NotificationCenter.default.addObserver(self, selector: #selector(timerNameReceivedFromPLC(_:)), name: NSNotification.Name(rawValue: NotificationKey.DidReceiveTimerFromGateway), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(timerParametarReceivedFromPLC(_:)), name: NSNotification.Name(rawValue: NotificationKey.DidReceiveTimerParameterFromGateway), object: nil)
    }
...
extension ScanCardsViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if let cell = tableView.dequeueReusableCell(withIdentifier: String(describing: CardsCell())) as? CardsCell {
            ...
            cell.setCell(card: cards[indexPath.row])
    ...
    func tableView(_ tableView: UITableView, editActionsForRowAt indexPath: IndexPath) -> [UITableViewRowAction]? {
        let button:UITableViewRowAction = UITableViewRowAction(style: UITableViewRowAction.Style.default, title: "Delete", handler: { (action:UITableViewRowAction, indexPath:IndexPath) in
            self.tableView(self.cardsTableView, commit: UITableViewCell.EditingStyle.delete, forRowAt: indexPath)
        })
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            appDel.managedObjectContext?.delete(cards[indexPath.row])
