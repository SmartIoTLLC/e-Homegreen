...
class IBeaconSettingsVC: UIViewController, UIGestureRecognizerDelegate {
                ...
                if uuidRegex.numberOfMatches(in: editUUID.text!, options: [], range: NSMakeRange(0, editUUID.text!.count)) > 0 {
                    if iBeacon == nil {
                        if let moc = appDel.managedObjectContext {
                            if let iBeaconNew = NSEntityDescription.insertNewObject(forEntityName: "IBeacon", into: moc) as? IBeacon {
...
extension IBeaconSettingsVC: UITextFieldDelegate {
