...
let HRToastTopMargin : CGFloat = 64.0

let HRToastPositionDefault  =   "top" //bottom
...
var HRToastTimer: UnsafePointer<Foundation.Timer>?          =   nil
...
extension UIView {
    ...
    */
    ...
    fileprivate func showToast(toast: UIView, duration: Double, position: AnyObject) {
        ...
        if existToast != nil {
            if let timer: Foundation.Timer = objc_getAssociatedObject(existToast, &HRToastTimer) as? Foundation.Timer {
                timer.invalidate();
            }
        ...
        UIView.animate(withDuration: HRToastFadeDuration,
            ...
            completion: { (finished: Bool) in
                let timer = Foundation.Timer.scheduledTimer(timeInterval: duration, target: self, selector: #selector(UIView.toastTimerDidFinish(_:)), userInfo: toast, repeats: false)
    ...
    fileprivate func centerPointForPosition(_ position: AnyObject, toast: UIView) -> CGPoint {
        if position is String {
            ...
            if position.lowercased == HRToastPositionTop {
                return CGPoint(x: viewSize.width/2, y: toastSize.height/2 + HRToastVerticalMargin + HRToastTopMargin)
