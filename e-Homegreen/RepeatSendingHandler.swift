...
class RepeatSendingHandler: NSObject {
    ...
    init(byteArray:[UInt8], gateway: Gateway, device:Device, oldValue:Int) {
        ...
        NSNotificationCenter.defaultCenter().addObserver(self, selector: "didGetResponseNotification:", name: NotificationKey.DidReceiveDataForRepeatSendingHandler, object: nil)
    }
        ...
        if !didGetResponse {
            if repeatCounter <= 4 {
                ...
                if let timer = didGetResponseTimer{
                    didGetResponseTimer.invalidate()
                    didGetResponseTimer = NSTimer.scheduledTimerWithTimeInterval(3, target: self, selector: "sendCommand", userInfo: nil, repeats: false)
                }else{
