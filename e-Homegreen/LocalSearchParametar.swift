...
class LocalSearchParametar {
    ...
    class func getLocalParametar(_ tab:String) ->[String] {
        ...
        if let a = userDefault.string(forKey: "\(tab)GatewaySearch") { stringForGatewaySearch = a }
        if let b = userDefault.string(forKey: "\(tab)LevelSearch") { stringForLevelSearch = b }
        if let c = userDefault.string(forKey: "\(tab)ZoneSearch") { stringForZoneSearch = c }
        if let d = userDefault.string(forKey: "\(tab)CategorySearch") { stringForCategorySearch = d }
        if let e = userDefault.string(forKey: "\(tab)LevelSearchName") { stringForLevelSearchName = e }
        if let f = userDefault.string(forKey: "\(tab)ZoneSearchName") { stringForZoneSearchName = f }
        if let g = userDefault.string(forKey: "\(tab)CategorySearchName") { stringForCategorySearchName = g }
