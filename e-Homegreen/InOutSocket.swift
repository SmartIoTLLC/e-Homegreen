...
class InOutSocket: NSObject, GCDAsyncUdpSocketDelegate {
    ...
    func setupConnection(){
        ...
        do { try socket.bind(toPort: port) } catch let error as NSError { print("1 \(String(describing: error))"); print("U pitanju je \(ip) \(port)") }
        do { try socket.beginReceiving() } catch let error as NSError { print("4 \(String(describing: error))"); print("U pitanju je \(ip) \(port)") }
        do { try socket.enableBroadcast(true) } catch let error as NSError { print("Error enabling broadcast :", error, error.userInfo); print("U pitanju je \(ip) \(port)") }
    }
